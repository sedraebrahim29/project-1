...
class _DoctorProfileScreenState extends State<DoctorProfileScreen> {
  ...
  void initState() {
    ...
    _days = List.generate(7, (i) => DateTime(today.year, today.month, today.day).add(Duration(days: i)));
    _selectedDate = _days.first;

    // تسجيل بيانات للـ Debugging
    debugPrint("Doctor ID: ${widget.doctor.id}");
    debugPrint("Clinic Refs Count: ${widget.doctor.clinicRefs.length}");

    _loadAvailability();
  }

  // تم تعديل هذا الجزء للتعامل مع String ID في حال كان هو المستخدم
  String get _doctorId => widget.doctor.id;
  ...
  Future<void> _loadAvailability() async {
    ...
    try {
      // تمرير الـ ID كـ String إذا كان الـ Repository يقبله، أو تحويله لـ int
      ...
      setState(() {
        _slots = slots;
        _isLoading = false;
        debugPrint("Successfully loaded ${_slots.length} slots.");
      });
    } catch (e) {
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = 'تعذر تحميل المواعيد: ${e.toString()}';
      });
    }
  }
                      ...
                      Padding(
                        ...
                        child: Center(
                          child: Text('No available slots for this day',
                              style: TextStyle(fontSize: bodyTextSize, color: AppColors.textLightGrey)),
                        ),
                      )
                    else ...[
                      if (morningSlots.isNotEmpty) ...[
                        Text(AppStrings.morning(context),
                        ...
                        Wrap(
                          ...
                          children: morningSlots
                              .map((s) => _buildTimeSlot(s, cardBgColor, borderColor, textColor, primaryGreenColor, doctorCardBg))
                              .toList(),
                        ),
                        SizedBox(height: 16.h),
                      ],
                      if (afternoonSlots.isNotEmpty) ...[
                        Text(AppStrings.afternoon(context),
                            style: TextStyle(fontSize: bodyTextSize, fontWeight: FontWeight.w600, color: isDark ? AppColors.darkText.withOpacity(0.6) : AppColors.textLightGrey)),
                        SizedBox(height: 8.h),
                        Wrap(
                          spacing: 10.w,
